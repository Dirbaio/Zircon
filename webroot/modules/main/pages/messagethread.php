...
function request($uid, $tid, $from=0)
{
    ...
    if($from == 0)
        Url::setCanonicalUrl('/u/#-:/messages/#-:', $user['id'], $user['name'], $thread['id'], $thread['title']);
    else
        Url::setCanonicalUrl('/u/#-:/messages/#-:/p#', $user['id'], $user['name'], $thread['id'], $thread['title'], $from);
    ...
    $breadcrumbs = array(
        ...
        array('user' => $user),
        array('url' => Url::format('/u/#-:/messages', $user['id'], $user['name']), 'title' => 'Messages'),
        array('url' => Url::format('/u/#-:/messages/#-:', $user['id'], $user['name'], $thread['id'], $thread['title']), 'title' => $thread['title']),
    );
    ...
    renderPage('messagethread.html', array(
        'props' => array(
            'draftType' => 4,
            'draftTarget' => $tid,
            'draft' => $draft,
