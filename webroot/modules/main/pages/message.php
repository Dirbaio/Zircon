...
function request($pid)
{
    ...
    if($from == 0)
        $url = Url::format('/u/#-:/messages/#-:', Session::id(), Session::get('name'), $thread['id'], $thread['title']);
    else
        $url = Url::format('/u/#-:/messages/#-:/p#', Session::id(), Session::get('name'), $thread['id'], $thread['title'], $from);
